...
fn generate_token() -> String {
    ...
    rand::rngs::OsRng.try_fill_bytes(&mut bytes).expect("OS RNG failed");
...
struct OAuthStore {
    ...
    authentication_states: Mutex<HashMap<String, (PasskeyAuthentication, PendingAuthApproval, u64)>>,
...
impl OAuthStore {
    ...
    fn new(config: OAuthConfig) -> Self {
        let rp_id = extract_domain(&config.server_url)
            .expect("invalid server_url: cannot extract domain");
        ...
        tracing::info!(
            ...
            passkey_store_path.parent().unwrap_or_else(|| Path::new(".")),
        );

        if passkeys.is_empty() {
            tracing::warn!("=== FIRST-TIME SETUP REQUIRED ===");
            tracing::warn!("No passkeys registered. Visit {}/passkey/register?setup_token=<SETUP_TOKEN> to register one.", config.server_url);
            ...
            tracing::info!("Server is locked: {} passkey(s) registered, {} dynamic client(s).", passkeys.len(), persisted.registered_clients.len());
        }
...
#[allow(clippy::similar_names, clippy::cognitive_complexity, clippy::needless_pass_by_value, clippy::too_many_lines)]
async fn authorize_get(
    ...
    let params: AuthorizeParams = match serde_urlencoded::from_str(query) {
        ...
        Err(e) => {
            ...
            AuthorizeParams {
                response_type: None, client_id: None, redirect_uri: None,
                state: None, code_challenge: None, code_challenge_method: None,
    ...
    {
        tracing::info!("Auto-approving /authorize via session cookie for client {}...", &client_id[..client_id.len().min(8)]);
        ...
        let mut redirect_url = Url::parse(redirect_uri).map_err(|_| {
            (StatusCode::BAD_REQUEST, Html(error_page(&store.config.app_name, "Invalid redirect URI.")))
        })?;
        {
        ...
        return Ok((StatusCode::FOUND, [(header::LOCATION, redirect_url.to_string())]).into_response());
    }
...
async fn token(
    ...
    tracing::info!("POST /token: grant_type={} client_id={}...", params.grant_type, &client_id[..client_id.len().min(8)]);

    if !store.validate_client(client_id, client_secret).await {
        let known = store.is_known_client(client_id).await;
        tracing::warn!("POST /token: invalid client credentials for client_id={}... (client known={})", &client_id[..client_id.len().min(8)], known);
        return Err((
...
async fn handle_refresh_token(
    ...
    let Some(entry) = store.refresh_tokens.lock().await.remove(refresh_token_val) else {
        tracing::warn!("Refresh token not found (already consumed or never existed), client_id={}...", &client_id[..client_id.len().min(8)]);
        return Err((
            StatusCode::BAD_REQUEST,
            Json(ErrorResponse {
                error: "invalid_grant".into(),
    ...
    if entry.client_id != client_id {
        tracing::warn!("Refresh token client_id mismatch: token belongs to {} but request from {}", &entry.client_id[..entry.client_id.len().min(8)], &client_id[..client_id.len().min(8)]);
        return Err((
            StatusCode::BAD_REQUEST,
            Json(ErrorResponse {
                error: "invalid_grant".into(),
    ...
    tracing::info!("Refresh token valid, issuing new tokens for client_id={}...", &client_id[..client_id.len().min(8)]);
...
async fn auth_middleware(
    ...
    let Some(h) = auth_header.filter(|h| h.len() > 7 && h[..7].eq_ignore_ascii_case("bearer ")) else {
    ...
    match tokens.get(token) {
        Some(entry) if now - entry.created_at < entry.expires_in_secs => {
            tracing::info!("Auth middleware: token {}... valid (age={}s)", token_prefix, now - entry.created_at);
            drop(tokens);
            ...
            if response.status() == StatusCode::UNAUTHORIZED {
                tracing::info!("Auth middleware: converting inner 401 to 404 (session not found, auth was valid)");
                return Ok((StatusCode::NOT_FOUND, "Session not found").into_response());
            }
            Ok(response)
        }
        Some(entry) => {
            tracing::warn!("Auth middleware: token {}... EXPIRED (age={}s, max={}s)", token_prefix, now - entry.created_at, entry.expires_in_secs);
            drop(tokens);
            Err(unauthorized_response(&store.config.server_url))
        }
        None => {
            tracing::warn!("Auth middleware: token {}... NOT FOUND ({} tokens in store)", token_prefix, tokens.len());
            drop(tokens);
            Err(unauthorized_response(&store.config.server_url))
        }
    }
...
async fn passkey_register_page(
    ...
    Html(register_page(&store.config.app_name, has_passkeys, query.setup_token.as_deref()))
}
...
mod tests {
    ...
    fn build_test_app(dir: &std::path::Path) -> Router {
        let protected = Router::new().route(
            "/mcp",
            get_route(|| async { "protected content" }),
        );
    ...
    async fn test_authorization_server_metadata() {
        ...
        assert_eq!(body["authorization_endpoint"], "https://mcp.example.com/authorize");
    ...
    async fn test_protected_route_invalid_token() {
        ...
        let resp = server
            .get("/mcp")
            .add_header(header::AUTHORIZATION, "Bearer invalid-token".parse::<axum::http::HeaderValue>().unwrap())
            .await;
        resp.assert_status(StatusCode::UNAUTHORIZED);
    }
    ...
    async fn test_authorize_valid_params_shows_setup_page() {
        ...
        resp.assert_status_ok();
        let body = resp.text();
        assert!(body.contains("setup") || body.contains("Setup") || body.contains("register") || body.contains("Register"));
    }
    ...
    async fn test_security_headers_present() {
        ...
        assert_eq!(resp.header("X-Content-Type-Options").to_str().unwrap(), "nosniff");
        assert_eq!(resp.header("Referrer-Policy").to_str().unwrap(), "no-referrer");
        assert!(resp.header("Content-Security-Policy").to_str().unwrap().contains("default-src 'self'"));
        assert!(resp.header("Permissions-Policy").to_str().unwrap().contains("camera=()"));
    }
    ...
    async fn test_pkce_code_verifier_too_short() {
        ...
        assert!(body["error_description"].as_str().unwrap().contains("43-128"));
    }
    ...
    fn test_authorize_page_xss_prevention() {
        ...
        let html = authorize_page("App", "</script><script>alert(1)", "https://r.com", "", "ch", "S256", "", true);
